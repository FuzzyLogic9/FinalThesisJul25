...
# GPT-2 setup
#gpt2_model_name = "gpt2"  # Change to "gpt2-medium", "gpt2-large", etc. if needed
...
# GPT-Neo setup
neo_model_name = "EleutherAI/gpt-neo-1.3B"  # or "EleutherAI/gpt-neo-125M"
gpt2_tokenizer = AutoTokenizer.from_pretrained(neo_model_name)
gpt2_tokenizer.pad_token = gpt2_tokenizer.eos_token
gpt2_model = AutoModelForCausalLM.from_pretrained(neo_model_name).to(device)
gpt2_model.eval()
torch.cuda.empty_cache()
...
def generate_text(prompt, max_new_tokens=None):
    ...
    inputs = gpt2_tokenizer(prompt, return_tensors="pt", truncation=True, padding=True, max_length=512)
    inputs = {k: v.to(device) for k, v in inputs.items()}
    input_len = inputs["input_ids"].shape[1]
    max_len = getattr(gpt2_model.config, "n_positions", getattr(gpt2_model.config, "max_position_embeddings", 1024))
    ...
    return gpt2_tokenizer.decode(output[0], skip_special_tokens=True).replace(prompt, '').strip()
...
text_only_toxic_file = base_path + "GPT2_NEO/text_output_escalated_GPT2NEO_G0_OT.csv"
scores_only_toxic_file = base_path + "GPT2_NEO/score_output_escalated_GPT2NEO_G0_OT.csv"
...
for _, row in tqdm(reviews_df.iterrows(), total=len(reviews_df)):
    ...
    for lvl in range(1, level + 1):
        ...
        if not toxic_generated:
            fallback = clean_generation(generate_text(f"React in a toxic, attacking, aggressive, hateful, harsh or rude way to this comment: {original}", 25))
...
with open(base_path + "GPT2_NEO/generation_summary_gpt2neo_g0_ot.txt", "a", encoding="utf-8") as f:
