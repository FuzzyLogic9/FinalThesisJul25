...
# ==================== GPU OPTIMISATION (RTX 3090) ====================
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"   # adaptive memory growth
torch.backends.cuda.matmul.allow_tf32 = True                         # enable TF32 on tensor cores
torch.set_float32_matmul_precision("high")                           # use fastest matmul path
torch.backends.cudnn.benchmark = True                                # optimise convolution kernels
...
# ==================== STANZA (CPU ONLY) ====================
stanza.download('en')
stanza_pipeline = stanza.Pipeline('en', processors='tokenize,sentiment', use_gpu=False)

# ==================== LLAMA SETUP ====================
llama_model_path = "C:/Users/GPUWIN-1/LLAMA"  
#llama_model_path = "C:/FuzzyNov2024/llama13b_finetuned_ksev_lora"

# 👉 replace with your local path if fine-tuned: e.g. C:/FuzzyNov2024/llama7b_finetuned_ksev_lora
...
def generate_text(prompt, max_new_tokens=None):
    ...
    inputs = llama_tokenizer(prompt, return_tensors="pt", truncation=True, padding=True, max_length=512)
    inputs = {k: v.to(device) for k, v in inputs.items()}

    input_len = inputs["input_ids"].shape[1]
    max_len = getattr(llama_model.config, "n_positions", getattr(llama_model.config, "max_position_embeddings", 4096))
    ...
    return llama_tokenizer.decode(output[0], skip_special_tokens=True).replace(prompt, '').strip()
...
text_only_toxic_file = base_path + "LLAMA/G0/text_output_escalated_LLAMA_G0_OT.csv"
scores_only_toxic_file = base_path + "LLAMA/G0/score_output_escalated_LLAMA_G0_OT.csv"
    ...
    for lvl in range(1, level + 1):
        ...
        for _ in range(10):
            ...
            toxic_prompt = improved_enrich_prompt(
                f"Continuing the discussion (keywords: {', '.join(orig_keywords)}):\n{context}", orig_keywords
            )
            gen = clean_generation(generate_text(toxic_prompt, random.randint(20, 35)))
            if (len(gen.split()) >= 8 and gen not in toxic_variants and keyword_overlap(orig_keywords, gen) and not id_pattern.search(gen)):
...
with open(base_path + "LLAMA/G0/generation_summary_llama_g0_ot.txt", "a", encoding="utf-8") as f:
...
print("✅ Saved toxicity-only responses with batch scoring using LLaMA.")
