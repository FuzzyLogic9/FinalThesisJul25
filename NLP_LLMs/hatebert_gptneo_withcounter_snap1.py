...
# Load GPTNEO_F1 (toxic model)
gptneo_path = "C:/FuzzyNov2024/gpt_neo13_finetuned_clean_KSEV"
gptneo_tokenizer = AutoTokenizer.from_pretrained(gptneo_path)
gptneo_tokenizer.pad_token = gptneo_tokenizer.eos_token
gptneo_model = AutoModelForCausalLM.from_pretrained(gptneo_path).to(device)
gptneo_model.eval()
torch.cuda.empty_cache()
...
def generate_text(prompt, max_new_tokens=None):
    ...
    inputs = gptneo_tokenizer(prompt, return_tensors="pt", truncation=True, padding=True, max_length=512)
    inputs = {k: v.to(device) for k, v in inputs.items()}
    input_len = inputs["input_ids"].shape[1]
    max_len = getattr(gptneo_model.config, "n_positions", getattr(gptneo_model.config, "max_position_embeddings", 1024))
    if input_len + max_new_tokens > max_len:
        inputs["input_ids"] = inputs["input_ids"][:, -max_len + max_new_tokens:]
    with torch.no_grad():
        output = gptneo_model.generate(
            **inputs,
            ...
            pad_token_id=gptneo_tokenizer.pad_token_id,
            eos_token_id=gptneo_tokenizer.eos_token_id
        )
    return gptneo_tokenizer.decode(output[0], skip_special_tokens=True).replace(prompt, '').strip()
...
text_only_toxic_file = base_path + "text_output_escalated_GPTNEO_G1_WC_snap.csv"
scores_only_toxic_file = base_path + "score_output_escalated_GPTNEO_G1_WC_snap.csv"
text_only_counter_file = base_path + "text_output_counter_GPTNEO.csv"
scores_only_counter_file = base_path + "score_output_counter_GPTNEO_G1_WC_snap.csv"
