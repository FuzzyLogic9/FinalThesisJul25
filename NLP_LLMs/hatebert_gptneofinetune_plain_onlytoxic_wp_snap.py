...
gptneo_model_path = "C:/FuzzyNov2024/gpt_neo13_finetuned_clean_KSEV"
gptneo_tokenizer = AutoTokenizer.from_pretrained(gptneo_model_path)
gptneo_tokenizer.pad_token = gptneo_tokenizer.eos_token
gptneo_model = AutoModelForCausalLM.from_pretrained(gptneo_model_path).to(device)
gptneo_model.eval()
...
def generate_text(prompt, max_new_tokens=None):
    ...
    full_text = gptneo_tokenizer.decode(output_ids[0], skip_special_tokens=True)
...
reviews_df = pd.read_csv(base_path + "group_zero_reviews_snap.csv")
...
text_only_toxic_file = base_path + "GPT2_NEO/text_output_escalated_GPTNEO_FT_G0_OT_snap.csv"
scores_only_toxic_file = base_path + "GPT2_NEO/score_output_escalated_GPTNEO_FT_G0_OT_snap.csv"
...
with open(base_path + "GPT2_NEO/generation_summary_gptneo_g0_ft_ot.txt", "a", encoding="utf-8") as f:
